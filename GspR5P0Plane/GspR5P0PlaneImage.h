 ...
 */
#ifndef GSPR5P0PLANE_GSPR5P0PLANEIMAGE_H_
#define GSPR5P0PLANE_GSPR5P0PLANEIMAGE_H_
...
class GspR5P0PlaneImage : public GspR5P0Plane {
public:
  GspR5P0PlaneImage(bool async, bool odd, bool support_up_down, bool size,
                    uint16_t up, uint16_t down, const GspRangeSize &range);
  virtual ~GspR5P0PlaneImage() {}
  ...
  struct gsp_r5p0_img_layer_user &getConfig();
...
private:
  ...
  bool checkImgLayerFormat(enum gsp_r5p0_img_layer_format format);
  ...
  void configFormat(enum gsp_r5p0_img_layer_format format);
  ...
  bool checkFBC(uint32_t pitch, uint32_t height,
                enum gsp_r5p0_img_layer_format format, bool inFBC);
  ...
  struct gsp_r5p0_img_layer_user mConfig;
};
} // namespace android

#endif // GSPR5P0PLANE_GSPR5P0PLANEIMAGE_H_
