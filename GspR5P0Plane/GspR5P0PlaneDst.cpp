...
GspR5P0PlaneDst::GspR5P0PlaneDst(bool async, int max_gspmmu_size,
                                 int max_gsp_bandwidth) {
  mAsync = async;
  maxGspMMUSize = max_gspmmu_size;
  maxGspBandwidth = max_gsp_bandwidth;

  ALOGI("create GspR5P0PlaneDst");
}

enum gsp_r5p0_des_layer_format GspR5P0PlaneDst::dstFormatConvert(int format) {
  enum gsp_r5p0_des_layer_format f = GSP_R5P0_DST_FMT_MAX_NUM;

  ALOGI_IF(mDebugFlag, "source dst format: 0x%x", format);

  switch (format) {
  case HAL_PIXEL_FORMAT_YCbCr_420_SP:
    f = GSP_R5P0_DST_FMT_YUV420_2P;
    break;
  case HAL_PIXEL_FORMAT_YCbCr_422_SP:
    f = GSP_R5P0_DST_FMT_YUV422_2P;
    break;
  case HAL_PIXEL_FORMAT_YV12:
    f = GSP_R5P0_DST_FMT_YUV420_3P;
    break;
  case HAL_PIXEL_FORMAT_RGBA_8888:
    f = GSP_R5P0_DST_FMT_ARGB888;
    break;
  case HAL_PIXEL_FORMAT_RGB_565:
    f = GSP_R5P0_DST_FMT_RGB565;
    break;
  case HAL_PIXEL_FORMAT_RGBX_8888:
    f = GSP_R5P0_DST_FMT_RGB888;
    break;
  default:
...
void GspR5P0PlaneDst::reset(int flag) {
  mDebugFlag = flag;
  memset(&mConfig, 0, sizeof(struct gsp_r5p0_des_layer_user));
}

bool GspR5P0PlaneDst::checkOutputRotation(SprdHWLayer **list, int count) {
  bool result = true;
...
bool GspR5P0PlaneDst::checkGspMMUSize(SprdHWLayer **list, int count) {
  bool result = true;

  native_handle_t *privateH = NULL;
  int i = 0;
  int needGspMmuSize = R5P0_DST_GSP_MMU_SIZE;
...
bool GspR5P0PlaneDst::checkGspBandwidth(SprdHWLayer **list, int count) {
  bool result = true;
...
bool GspR5P0PlaneDst::adapt(SprdHWLayer **list, int count) {
...
void GspR5P0PlaneDst::configCommon(int wait_fd, int share_fd) {
...
void GspR5P0PlaneDst::configFormat(int format) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;

  params->img_format = format;
}

void GspR5P0PlaneDst::configFBC(bool outAFBC, int format) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
  uint32_t fbc_mode;

  if (outAFBC && ((GSP_R5P0_DST_FMT_ARGB888 == format) ||
                  (GSP_R5P0_DST_FMT_RGB888 == format)))
    fbc_mode = 2;
  else
    fbc_mode = 0;
...
void GspR5P0PlaneDst::configPitch(uint32_t pitch) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
...
void GspR5P0PlaneDst::configHeight(uint32_t height) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
...
void GspR5P0PlaneDst::configEndian(int f, uint32_t w, uint32_t h) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
...
void GspR5P0PlaneDst::configBackGround(struct gsp_background_para bg_para) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
...
void GspR5P0PlaneDst::configOutputRotation(enum gsp_rot_angle rot_angle) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
  params->rot_angle = rot_angle;
}

void GspR5P0PlaneDst::configDither(bool enable) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
  params->dither_en = enable;
}

void GspR5P0PlaneDst::configCSCMode(uint8_t r2y_mod) {
  struct gsp_r5p0_des_layer_params *params = &mConfig.params;
  params->r2y_mod = r2y_mod;
}

struct gsp_r5p0_des_layer_user &GspR5P0PlaneDst::getConfig() {
  return mConfig;
}

bool GspR5P0PlaneDst::parcel(native_handle_t *handle, uint32_t w, uint32_t h,
