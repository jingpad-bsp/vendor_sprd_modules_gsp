...
GspR5P0PlaneArray::GspR5P0PlaneArray(bool async) {
  mAsync = async;
  mSplitSupport = 0;
  mDebugFlag = 0;
  mInputRotMode = false;
...
GspR5P0PlaneArray::~GspR5P0PlaneArray() {
  for (int i = 0; i < R5P0_IMGL_NUM; i++) {
    if (mImagePlane[i])
      delete mImagePlane[i];
  }
  for (int i = 0; i < R5P0_OSDL_NUM; i++) {
    if (mOsdPlane[i])
      delete mOsdPlane[i];
  }
...
int GspR5P0PlaneArray::init(int fd) {
  int ret = -1;

  ret = ioctl(fd, GSP_IO_GET_CAPABILITY(struct gsp_r5p0_capability),
              &mCapability);
  if (ret < 0) {
    ALOGE("gspr5p0plane array get capability failed, ret=%d.\n", ret);
    return ret;
  }
  ...
  for (int i = 0; i < R5P0_IMGL_NUM; i++) {
    mImagePlane[i] = new GspR5P0PlaneImage(
    ...
    if (mImagePlane[i] == NULL) {
      ALOGE("R5P0 ImgPlane alloc fail!");
      return -1;
    }
  }

  for (int i = 0; i < R5P0_OSDL_NUM; i++) {
    mOsdPlane[i] = new GspR5P0PlaneOsd(mAsync, range);
    if (mOsdPlane[i] == NULL) {
      ALOGE("R5P0 OsdPlane[%d] alloc fail!", i);
      return -1;
    }
  }

  mDstPlane = new GspR5P0PlaneDst(mAsync, mCapability.max_gspmmu_size,
                                  mCapability.max_gsp_bandwidth);
  if (mDstPlane == NULL) {
    ALOGE("R5P0 DstPlane alloc fail!");
    return -1;
  }

  mPlaneNum = R5P0_IMGL_NUM + R5P0_OSDL_NUM;

  mConfigs = new gsp_r5p0_cfg_user[mCapability.common.io_cnt];
  if (mConfigs == NULL) {
    ALOGE("R5P0 mConfigs alloc fail, io_cnt:%d !", mCapability.common.io_cnt);
    return -1;
  }
...
void GspR5P0PlaneArray::queryDebugFlag(int *debugFlag) {
...
bool GspR5P0PlaneArray::checkLayerCount(int count) {
  return count > mPlaneNum ? false : true;
}

bool GspR5P0PlaneArray::isExhausted() {
  return mAttachedNum == mMaxAttachedNum ? true : false;
}

bool GspR5P0PlaneArray::isSupportBufferType(enum gsp_addr_type type) {
...
bool GspR5P0PlaneArray::isSupportSplit() { return mSplitSupport; }

void GspR5P0PlaneArray::planeAttached() {
  mAttachedNum++;
  ALOGI_IF(mDebugFlag, "has attached total %d plane", mAttachedNum);
}

bool GspR5P0PlaneArray::isImagePlaneAttached(int ImgPlaneIndex) {
  return mImagePlane[ImgPlaneIndex]->isAttached();
}

bool GspR5P0PlaneArray::isOsdPlaneAttached(int OsdPlaneIndex) {
  return mOsdPlane[OsdPlaneIndex]->isAttached();
}

bool GspR5P0PlaneArray::imagePlaneAdapt(SprdHWLayer *layer, int ImgPlaneIndex,
...
bool GspR5P0PlaneArray::osdPlaneAdapt(SprdHWLayer *layer, int OsdPlaneIndex,
                                      int LayerIndex) {
...
bool GspR5P0PlaneArray::dstPlaneAdapt(SprdHWLayer **list, int count) {
  return mDstPlane->adapt(list, count);
}

bool GspR5P0PlaneArray::needImgLayer(SprdHWLayer *layer) {
...
bool GspR5P0PlaneArray::needRotation(SprdHWLayer *layer) {
  uint32_t transform = layer->getTransform();

  return transform == 0 ? false : true;
}

int GspR5P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
                             uint32_t h) {
  int DPULayerNum = 0;
  int i = 0;
  int ret = 0;
...
int GspR5P0PlaneArray::full_adapt(SprdHWLayer **list, int count, uint32_t w,
                                  uint32_t h) {
  int ret = 0;
  bool result = false;
  ...
  for (i = 0; i < count; i++) {
    if (needImgLayer(list[i]) == true) {
      ...
      if (img_index0 < R5P0_IMGL_NUM - yuv_index) {
  ...
  if (result == false) {
    if (!input_rot_adapt(list, count)) {
      mInputRotMode = true;
      ALOGI_IF(mDebugFlag, "gsp r5p0 input rotation mode.");
    } else {
  ...
  ALOGI_IF(mDebugFlag, "gsp r5p0plane full adapt success");

  return ret;
}

int GspR5P0PlaneArray::partial_adapt(SprdHWLayer **list, int count, uint32_t w,
                                     uint32_t h) {
  int ret = 0;
  bool result = false;
  native_handle_t *privateH = NULL;
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (list[i]->getAccelerator() != ACCELERATOR_DISPC) {
      ...
      img_index++;
      ALOGI_IF(mDebugFlag, "gsp r5p0 partial adapt success");
      return 0;
    }
  }

  ALOGI_IF(mDebugFlag, "gsp r5p0 partial adapt: strange to go here!");
  return ret;
}

int GspR5P0PlaneArray::input_rot_adapt(SprdHWLayer **list, int count) {
  int ret = 0;
  bool result = false;
  int32_t yuv_index = 0; // yuv layer index
  int32_t osd_index = 0;
  ...
  ALOGI_IF(mDebugFlag, "gsp r5p0 input rotation adapt success");
  return ret;
}

bool GspR5P0PlaneArray::imagePlaneParcel(SprdHWLayer **list, int ImgPlaneIndex,
                                         bool InputRotMode) {
...
bool GspR5P0PlaneArray::osdPlaneParcel(SprdHWLayer **list, int OsdPlaneIndex) {
...
bool GspR5P0PlaneArray::dstPlaneParcel(native_handle_t *handle, uint32_t w,
                                       uint32_t h, int format, int wait_fd,
                                       int32_t angle) {
  return mDstPlane->parcel(handle, w, h, format, wait_fd, angle);
}

int GspR5P0PlaneArray::get_tap_var0(int srcPara, int destPara) {
  int retTap = 0;
  ...
  retTap = (8 - retTap) / 2;
  // retTap = 3 is null for htap & vtap on r5p0

  return retTap;
}

bool GspR5P0PlaneArray::miscCfgParcel(int mode_type, uint32_t transform) {
  bool status = false;
  int icnt = 0;
  struct gsp_r5p0_misc_cfg_user &r5p0_misc_cfg = mConfigs[mConfigIndex].misc;
  struct gsp_r5p0_img_layer_user r5p0_limg_cfg;
  uint32_t dstw = 0;
  uint32_t dsth = 0;
  ...
  case 0: {
    ...
    r5p0_misc_cfg.run_mod = 0;
    r5p0_misc_cfg.scale_seq = 0;

    for (icnt = 0; icnt < R5P0_IMGL_NUM; icnt++) {
      r5p0_limg_cfg = mImagePlane[icnt]->getConfig();
      if (r5p0_limg_cfg.params.scaling_en) {
        r5p0_misc_cfg.scale_para[icnt].scale_en = 1;
      } else {
        r5p0_misc_cfg.scale_para[icnt].scale_en = 0;
      }
      r5p0_misc_cfg.scale_para[icnt].scale_rect_in =
          r5p0_limg_cfg.params.clip_rect;
      r5p0_misc_cfg.scale_para[icnt].scale_rect_out =
          r5p0_limg_cfg.params.des_rect;

      if (transform & HAL_TRANSFORM_ROT_90) {
        dstw = r5p0_misc_cfg.scale_para[icnt].scale_rect_out.rect_h;
        dsth = r5p0_misc_cfg.scale_para[icnt].scale_rect_out.rect_w;
      } else {
        dstw = r5p0_misc_cfg.scale_para[icnt].scale_rect_out.rect_w;
        dsth = r5p0_misc_cfg.scale_para[icnt].scale_rect_out.rect_h;
      }

      r5p0_misc_cfg.scale_para[icnt].htap_mod = get_tap_var0(
          r5p0_misc_cfg.scale_para[icnt].scale_rect_in.rect_w, dstw);

      r5p0_misc_cfg.scale_para[icnt].vtap_mod = get_tap_var0(
          r5p0_misc_cfg.scale_para[icnt].scale_rect_in.rect_h, dsth);
    }

    r5p0_misc_cfg.workarea1_src_rect.st_x = 0;
    r5p0_misc_cfg.workarea1_src_rect.st_y = 0;
    r5p0_misc_cfg.workarea1_src_rect.rect_w = mOutputWidth;
    r5p0_misc_cfg.workarea1_src_rect.rect_h = mOutputHeight;
    r5p0_misc_cfg.workarea2_src_rect.st_x = 0;
    r5p0_misc_cfg.workarea2_src_rect.st_y = 0;
    r5p0_misc_cfg.workarea2_src_rect.rect_w = mOutputWidth;
    r5p0_misc_cfg.workarea2_src_rect.rect_h = mOutputHeight;
    r5p0_misc_cfg.workarea2_des_pos.pt_x = 0;
    r5p0_misc_cfg.workarea2_des_pos.pt_y = 0;
  }
    status = true;
    break;
  default:
    ALOGE("gsp r5p0 not implement other mode(%d) yet! ", mode_type);
    break;
  }
...
int GspR5P0PlaneArray::split(SprdHWLayer **list, int count) {
...
int GspR5P0PlaneArray::rotAdjustSingle(uint16_t *dx, uint16_t *dy, uint16_t *dw,
...
int GspR5P0PlaneArray::rotAdjust(struct gsp_r5p0_cfg_user *cmd_info,
  ...
  int32_t icnt = 0;
  struct gsp_r5p0_osd_layer_user *osd_info = NULL;
  struct gsp_r5p0_img_layer_user *img_info = NULL;
  ...
  for (icnt = 0; icnt < R5P0_OSDL_NUM; icnt++) {
...
bool GspR5P0PlaneArray::parcel(SprdUtilSource *source, SprdUtilTarget *target) {
...
int GspR5P0PlaneArray::run() {
  unsigned long value = GSP_R5P0_PLANE_TRIGGER;
  int ret = -1;

  return ioctl(mDevice, value, mConfigs);
}

int GspR5P0PlaneArray::acquireSignalFd() {
  return mConfigs[mConfigIndex].ld1.common.sig_fd;
}

int GspR5P0PlaneArray::set(SprdUtilSource *source, SprdUtilTarget *target,
  ...
  if (parcel(source, target) == false) {
    ALOGE("gsp r5p0plane parcel failed");
    return ret;
  }
