...
int GspR4P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
                             uint32_t h) {
  int ret = 0;
  bool result = false;
...
bool GspR4P0PlaneArray::miscCfgParcel(int mode_type, uint32_t transform) {
  ...
  case 0: {
    ...
    for (icnt = 0; icnt < R4P0_IMGL_NUM; icnt++) {
      ...
      r4p0_misc_cfg.scale_para[icnt].htap_mod = get_tap_var0(
          r4p0_misc_cfg.scale_para[icnt].scale_rect_in.rect_w, dstw);

      r4p0_misc_cfg.scale_para[icnt].vtap_mod = get_tap_var0(
          r4p0_misc_cfg.scale_para[icnt].scale_rect_in.rect_h, dsth);
    }
