 ...
 */
#ifndef GSPR4P0PLANE_GSPR4P0PLANEIMAGE_H_
#define GSPR4P0PLANE_GSPR4P0PLANEIMAGE_H_
...
class GspR4P0PlaneImage : public GspR4P0Plane {
public:
  GspR4P0PlaneImage(bool async, bool odd, bool support_up_down, bool size,
                    uint16_t up, uint16_t down, const GspRangeSize &range);
  virtual ~GspR4P0PlaneImage() {}
  ...
  struct gsp_r4p0_img_layer_user &getConfig();
...
private:
  ...
  void configFormat(enum gsp_r4p0_img_layer_format format);
  ...
  bool checkFBC(uint32_t pitch, uint32_t height,
                enum gsp_r4p0_img_layer_format format, bool inFBC);
  ...
  struct gsp_r4p0_img_layer_user mConfig;
};
} // namespace android

#endif // GSPR4P0PLANE_GSPR4P0PLANEIMAGE_H_
