...
GspR4P0PlaneOsd::GspR4P0PlaneOsd(bool async, const GspRangeSize &range) {
  mIndex = -1;
  mAttached = false;
  mAsync = async;
  mRangeSize = range;
  mRangeSize.print();
  ALOGI("create GspR4P0PlaneOsd");
}

void GspR4P0PlaneOsd::reset(int flag) {
  mAttached = false;
  mIndex = -1;
  mDebugFlag = flag;
  memset(&mConfig, 0, sizeof(struct gsp_r4p0_osd_layer_user));
}

bool GspR4P0PlaneOsd::checkFBC(uint32_t pitch, uint32_t height,
                               enum gsp_r4p0_osd_layer_format format,
                               bool inFBC) {
  bool isIFBC = false;
  bool isVFBC = false;
  ...
  if (inFBC) {
    if ((GSP_R4P0_OSD_FMT_ARGB888 == format) ||
        (GSP_R4P0_OSD_FMT_RGB888 == format)) {
      isIFBC = true;
      horAlign = 16;
      verAlign = 4;
    } else {
...
bool GspR4P0PlaneOsd::adapt(SprdHWLayer *layer, int LayerIndex) {
  ...
  enum gsp_r4p0_osd_layer_format format = osdFormatConvert(ADP_FORMAT(handle));
...
void GspR4P0PlaneOsd::configCommon(int wait_fd, int share_fd, int enable) {
...
void GspR4P0PlaneOsd::configSize(struct sprdRect *srcRect,
                                 struct sprdRect *dstRect, uint32_t pitch,
                                 uint32_t height) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
...
void GspR4P0PlaneOsd::configSize(uint32_t w, uint32_t h) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
...
void GspR4P0PlaneOsd::configFormat(enum gsp_r4p0_osd_layer_format format) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
...
void GspR4P0PlaneOsd::configFBC(bool inFBC, int format) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
  uint32_t fbc_mode;

  if (inFBC && ((GSP_R4P0_OSD_FMT_ARGB888 == format) ||
                (GSP_R4P0_OSD_FMT_RGB888 == format)))
    fbc_mode = 1;
  else
    fbc_mode = 0;
...
void GspR4P0PlaneOsd::configPmargbMode(uint32_t blendMode) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
...
void GspR4P0PlaneOsd::configAlpha(uint8_t alpha) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
...
void GspR4P0PlaneOsd::configZorder(uint8_t zorder) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
...
void GspR4P0PlaneOsd::configEndian(native_handle_t *handle) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
  ...
  params->endian.y_rgb_qword_endn = GSP_R4P0_QWORD_ENDN_0;
...
void GspR4P0PlaneOsd::configPallet(int enable) {
  struct gsp_r4p0_osd_layer_params *params = &mConfig.params;

  params->pallet_en = enable;
}

struct gsp_r4p0_osd_layer_user &GspR4P0PlaneOsd::getConfig() {
  return mConfig;
}

bool GspR4P0PlaneOsd::parcel(SprdHWLayer *layer) {
  if (layer == NULL) {
...
bool GspR4P0PlaneOsd::parcel(uint32_t w, uint32_t h) {
  configPallet(1);
