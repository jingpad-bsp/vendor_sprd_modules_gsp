...
enum gsp_lite_r2p0_img_layer_format GspLiteR2P0Plane::imgFormatConvert(int f) {
  enum gsp_lite_r2p0_img_layer_format format = GSP_LITE_R2P0_IMG_FMT_MAX_NUM;

  ALOGI_IF(mDebugFlag, "source format: %d", f);
  switch (f) {
  case HAL_PIXEL_FORMAT_YCbCr_420_SP: // 0x19
    format = GSP_LITE_R2P0_IMG_FMT_YUV420_2P;
    break;
  ...
  case HAL_PIXEL_FORMAT_YV12:
    format = GSP_LITE_R2P0_IMG_FMT_YV12;
    break;
  case HAL_PIXEL_FORMAT_RGBA_8888:
    format = GSP_LITE_R2P0_IMG_FMT_ARGB888;
    break;
  case HAL_PIXEL_FORMAT_RGBX_8888:
    format = GSP_LITE_R2P0_IMG_FMT_RGB888;
    break;
  case HAL_PIXEL_FORMAT_RGB_565:
    format = GSP_LITE_R2P0_IMG_FMT_RGB565;
    break;
  case HAL_PIXEL_FORMAT_YCbCr_422_SP:
    format = GSP_LITE_R2P0_IMG_FMT_YUV422_2P;
    break;
...
enum gsp_lite_r2p0_osd_layer_format GspLiteR2P0Plane::osdFormatConvert(int f) {
  enum gsp_lite_r2p0_osd_layer_format format = GSP_LITE_R2P0_OSD_FMT_MAX_NUM;

  ALOGI_IF(mDebugFlag, "source osd format: %d", f);
  switch (f) {
  case HAL_PIXEL_FORMAT_RGBA_8888:
    format = GSP_LITE_R2P0_OSD_FMT_ARGB888;
    break;
  case HAL_PIXEL_FORMAT_RGBX_8888:
    format = GSP_LITE_R2P0_OSD_FMT_RGB888;
    break;
  case HAL_PIXEL_FORMAT_RGB_565:
    format = GSP_LITE_R2P0_OSD_FMT_RGB565;
    break;
  default:
...
enum gsp_rot_angle GspLiteR2P0Plane::rotationTypeConvert(int32_t angle) {
...
bool GspLiteR2P0Plane::checkRangeSize(struct sprdRect *srcRect,
...
bool GspLiteR2P0Plane::checkBlending(SprdHWLayer *layer) {
  bool result = false;
  ...
  case HWC_BLENDING_COVERAGE:
    ALOGI_IF(mDebugFlag, "lite_r2p0 support blending flag: %d",
             layer->getBlendMode());
    result = true;
    break;
  default:
...
bool GspLiteR2P0Plane::isLandScapeTransform(enum gsp_rot_angle rot) {
  bool result = false;
...
bool GspLiteR2P0Plane::needScale(struct sprdRect *srcRect,
                                 struct sprdRect *dstRect,
                                 enum gsp_rot_angle rot) {
  bool result = false;
...
bool GspLiteR2P0Plane::isVideoLayer(
    enum gsp_lite_r2p0_img_layer_format format) {
  bool result = false;

  if (format >= GSP_LITE_R2P0_IMG_FMT_YUV422_2P &&
      format <= GSP_LITE_R2P0_IMG_FMT_YUV420_3P)
    result = true;
...
bool GspLiteR2P0Plane::isVideoLayer(
  ...
  if (format > GSP_LITE_R2P0_OSD_FMT_RGB565)
    result = true;
