...
int GspLiteR2P0PlaneArray::init(int fd) {
  ...
  drm_cap.size = sizeof(gsp_lite_r2p0_capability);
  ...
  drm_mConfigs =
      (struct drm_gsp_lite_r2p0_cfg_user *)malloc(sizeof(drm_gsp_lite_r2p0_cfg_user));
  drm_mConfigs->async = mAsync;
  drm_mConfigs->config = mConfigs;
  drm_mConfigs->size = sizeof(gsp_lite_r2p0_cfg_user);
  drm_mConfigs->num = 1;
...
bool GspLiteR2P0PlaneArray::isYuvLayer(SprdHWLayer *layer) {
  ...
  privateH = layer->getBufferHandle();
...
bool GspLiteR2P0PlaneArray::dstPlaneParcel(native_handle_t *handle, uint32_t w,
...
int GspLiteR2P0PlaneArray::rotAdjust(struct gsp_lite_r2p0_cfg_user *cmd_info,
  ...
  for (icnt = 0; icnt < LITE_R2P0_IMGL_NUM; icnt++) {
    if (img_info[icnt].common.enable == 1) {
      ...
      if (cmd_info->misc.scale_para.scale_en == 1) {
        cmd_info->misc.scale_para.htap_mod =
            get_tap_var0(cmd_info->misc.scale_para.scale_rect_in.rect_w,
                         cmd_info->misc.scale_para.scale_rect_out.rect_w);

        cmd_info->misc.scale_para.vtap_mod =
            get_tap_var0(cmd_info->misc.scale_para.scale_rect_in.rect_h,
                         cmd_info->misc.scale_para.scale_rect_out.rect_h);
      }
      if (ret) {
