...
GspLiteR2P0PlaneImage::GspLiteR2P0PlaneImage(bool async, bool is_need_even,
  ...
  ALOGI("create GspLiteR2P0PlaneImage");
  mRangeSize.print();
...
//TODO: depend on gralloc USC_YUV_INFO patch
#if 0
bool GspLiteR2P0PlaneImage::checkYuvInfo(mali_gralloc_yuv_info info) {
  bool result = true;
...
bool GspLiteR2P0PlaneImage::checkVideoSize(struct sprdRect *srcRect) {
  bool result = true;
...
bool GspLiteR2P0PlaneImage::checkOddBoundary(
    struct sprdRect *srcRect, enum gsp_lite_r2p0_img_layer_format format) {
  bool result = true;
...
bool GspLiteR2P0PlaneImage::needScaleUpDown(struct sprdRect *srcRect,
...
bool GspLiteR2P0PlaneImage::checkScaleSize(struct sprdRect *srcRect,
                                           struct sprdRect *dstRect,
                                           enum gsp_rot_angle rot) {
  bool result = true;
...
bool GspLiteR2P0PlaneImage::checkScale(struct sprdRect *srcRect,
                                       struct sprdRect *dstRect,
                                       enum gsp_rot_angle rot) {
  if (needScaleUpDown(srcRect, dstRect, rot) == true)
    return false;

  if (checkScaleSize(srcRect, dstRect, rot) == false)
    return false;
...
#if 0
bool GspLiteR2P0PlaneImage::checkYuvType(mali_gralloc_yuv_info yuv_info) {
  bool result = true;
...
bool GspLiteR2P0PlaneImage::checkImgLayerFormat(
    enum gsp_lite_r2p0_img_layer_format format) {
  bool result = true;

  if (format == GSP_LITE_R2P0_IMG_FMT_MAX_NUM) {
...
void GspLiteR2P0PlaneImage::reset(int flag) {
  mAttached = false;
  mIndex = -1;
  mDebugFlag = flag;
  memset(&mConfig, 0, sizeof(struct gsp_lite_r2p0_img_layer_user));
}

bool GspLiteR2P0PlaneImage::adapt(SprdHWLayer *layer, int LayerIndex) {
  ...
  if (checkScale(srcRect, dstRect, rot) == false)
    return false;
...
void GspLiteR2P0PlaneImage::configCommon(int wait_fd, int share_fd) {
...
void GspLiteR2P0PlaneImage::configScaleEnable(struct sprdRect *srcRect,
                                              struct sprdRect *dstRect,
                                              enum gsp_rot_angle rot) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
...
void GspLiteR2P0PlaneImage::configSize(
    struct sprdRect *srcRect, struct sprdRect *dstRect, uint32_t pitch,
    uint32_t height, enum gsp_lite_r2p0_img_layer_format format,
    enum gsp_rot_angle rot) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
...
void GspLiteR2P0PlaneImage::configFormat(
    enum gsp_lite_r2p0_img_layer_format format) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
...
void GspLiteR2P0PlaneImage::configPmargbMode(uint32_t blendMode) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
...
void GspLiteR2P0PlaneImage::configAlpha(uint8_t alpha) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
...
#if 0
void GspLiteR2P0PlaneImage::configCSCMatrix(mali_gralloc_yuv_info info) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
  uint8_t y2r_mod = 0;
  switch (info) {
...
void GspLiteR2P0PlaneImage::configZorder(uint8_t zorder) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
...
void GspLiteR2P0PlaneImage::configEndian(native_handle_t *handle) {
  struct gsp_lite_r2p0_img_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
  ...
  case HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED:
    params->endian.uv_word_endn = GSP_LITE_R2P0_WORD_ENDN_3;
    break;
  case HAL_PIXEL_FORMAT_YCbCr_420_SP:
    params->endian.uv_word_endn = GSP_LITE_R2P0_WORD_ENDN_0;
    break;
...
struct gsp_lite_r2p0_img_layer_user &GspLiteR2P0PlaneImage::getConfig() {
  return mConfig;
}

bool GspLiteR2P0PlaneImage::parcel(SprdHWLayer *layer) {
  if (layer == NULL) {
  ...
  configSize(srcRect, dstRect, pitch, ADP_HEIGHT(handle), format, rot);
