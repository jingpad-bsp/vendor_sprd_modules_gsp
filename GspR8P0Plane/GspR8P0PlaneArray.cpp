...
#include "GspR8P0PlaneArray.h"
#include <algorithm>
#include "GspR8P0PlaneImage.h"
#include "GspR8P0PlaneOsd.h"
...
GspR8P0PlaneArray::GspR8P0PlaneArray(bool async) {
  mAsync = async;
  mSplitSupport = 0;
  mDebugFlag = 0;
  mInputRotMode = false;
...
GspR8P0PlaneArray::~GspR8P0PlaneArray() {
  for (int i = 0; i < R8P0_IMGL_NUM; i++) {
    if (mImagePlane[i])
      delete mImagePlane[i];
  }
  for (int i = 0; i < R8P0_OSDL_NUM; i++) {
    if (mOsdPlane[i])
      delete mOsdPlane[i];
  }
...
int GspR8P0PlaneArray::init(int fd) {
  int ret = -1;
  struct drm_gsp_capability drm_cap;

  drm_cap.size = sizeof(gsp_r8p0_capability);
  drm_cap.cap = &mCapability;

  ret = drmIoctl(fd, DRM_IOCTL_SPRD_GSP_GET_CAPABILITY, &drm_cap);
  if (ret < 0) {
    ALOGE("gspr8p0plane array get capability failed, ret=%d.\n", ret);
    return ret;
  }
  ...
  for (int i = 0; i < R8P0_IMGL_NUM; i++) {
    mImagePlane[i] = new GspR8P0PlaneImage(
    ...
    if (mImagePlane[i] == NULL) {
      ALOGE("R8P0 ImgPlane alloc fail!");
      return -1;
    }
  }

  for (int i = 0; i < R8P0_OSDL_NUM; i++) {
    mOsdPlane[i] = new GspR8P0PlaneOsd(mAsync, range);
    if (mOsdPlane[i] == NULL) {
      ALOGE("R8P0 OsdPlane[%d] alloc fail!", i);
      return -1;
    }
  }

  mDstPlane = new GspR8P0PlaneDst(mAsync, mCapability.max_gspmmu_size,
                                  mCapability.max_gsp_bandwidth);
  if (mDstPlane == NULL) {
    ALOGE("R8P0 DstPlane alloc fail!");
    return -1;
  }

  mPlaneNum = R8P0_IMGL_NUM + R8P0_OSDL_NUM;

  mConfigs = new gsp_r8p0_cfg_user[mCapability.common.io_cnt];
  if (mConfigs == NULL) {
    ALOGE("R8P0 mConfigs alloc fail, io_cnt:%d !", mCapability.common.io_cnt);
    return -1;
  }

  drm_mConfigs =
      (struct drm_gsp_r8p0_cfg_user *)malloc(sizeof(drm_gsp_r8p0_cfg_user));
  drm_mConfigs->async = mAsync;
  drm_mConfigs->config = mConfigs;
  drm_mConfigs->size = sizeof(gsp_r8p0_cfg_user);
  drm_mConfigs->num = 1;
...
void GspR8P0PlaneArray::reset() {
  ...
  memset(&mConfigs->misc, 0, sizeof(struct gsp_r8p0_misc_cfg_user));
}

void GspR8P0PlaneArray::queryDebugFlag(int *debugFlag) {
...
bool GspR8P0PlaneArray::checkLayerCount(int count) {
  return count > mPlaneNum ? false : true;
}

bool GspR8P0PlaneArray::isExhausted() {
  return mAttachedNum == mMaxAttachedNum ? true : false;
}

bool GspR8P0PlaneArray::isSupportBufferType(enum gsp_addr_type type) {
...
bool GspR8P0PlaneArray::isSupportSplit() { return mSplitSupport; }

void GspR8P0PlaneArray::planeAttached() {
  mAttachedNum++;
  ALOGI_IF(mDebugFlag, "has attached total %d plane", mAttachedNum);
}

bool GspR8P0PlaneArray::isImagePlaneAttached(int ImgPlaneIndex) {
  return mImagePlane[ImgPlaneIndex]->isAttached();
}

bool GspR8P0PlaneArray::isOsdPlaneAttached(int OsdPlaneIndex) {
  return mOsdPlane[OsdPlaneIndex]->isAttached();
}

bool GspR8P0PlaneArray::imagePlaneAdapt(SprdHWLayer *layer, int count,
                                        int ImgPlaneIndex, int LayerIndex,
                                        bool InputRotMode) {
  if (isImagePlaneAttached(ImgPlaneIndex) == true)
    return false;

  return mImagePlane[ImgPlaneIndex]->adapt(layer, count, LayerIndex,
                                           InputRotMode);
}

bool GspR8P0PlaneArray::osdPlaneAdapt(SprdHWLayer *layer, int OsdPlaneIndex,
                                      int LayerIndex) {
...
bool GspR8P0PlaneArray::dstPlaneAdapt(SprdHWLayer **list, int count) {
  return mDstPlane->adapt(list, count);
}

bool GspR8P0PlaneArray::needImgLayer(SprdHWLayer *layer) {
  ...
  if ((ADP_FORMAT(handle) == HAL_PIXEL_FORMAT_YCbCr_420_SP) ||
      ...
      (ADP_FORMAT(handle) == HAL_PIXEL_FORMAT_YCbCr_420_888) ||
...
bool GspR8P0PlaneArray::needRotation(SprdHWLayer *layer) {
  uint32_t transform = layer->getTransform();

  return transform == 0 ? false : true;
}

int GspR8P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
                             uint32_t h) {
  int DPULayerNum = 0;
  int i = 0;
  int ret = 0;
...
int GspR8P0PlaneArray::gsp_choose_layer(SprdHWLayer **list, int count) {
  int ret = 0;
  int i, j = 0;
  int DPULayerNum = 0;
  int skip_index = -1;
  ...
  if (count - DPULayerNum > R8P0_IMGL_NUM + R8P0_OSDL_NUM) {
...
int GspR8P0PlaneArray::gsp_adapt(SprdHWLayer **list, int count, uint32_t w,
                                 uint32_t h) {
  int ret = 0;
  bool result = false;
  ...
  if (yuv_index + input_rot_num > R8P0_IMGL_NUM) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (needImgLayer(list[i]) || input_rot_mode) {
      ...
      if (img_index0 < (R8P0_IMGL_NUM - (yuv_index + input_rot_num))) {
        result = imagePlaneAdapt(list[i], count,
                                 img_index0 + yuv_index + input_rot_num,
                                 layer_index, false);
        if (result == false) {
  ...
  ALOGI_IF(mDebugFlag, "gsp r8p0plane full adapt success");

  return ret;
}

bool GspR8P0PlaneArray::imagePlaneParcel(SprdHWLayer **list, int ImgPlaneIndex,
                                         bool InputRotMode) {
...
bool GspR8P0PlaneArray::osdPlaneParcel(SprdHWLayer **list, int OsdPlaneIndex) {
...
bool GspR8P0PlaneArray::dstPlaneParcel(native_handle_t *handle, uint32_t w,
                                       uint32_t h, int format, int wait_fd,
                                       int32_t angle) {
  return mDstPlane->parcel(handle, w, h, format, wait_fd, angle);
}

int GspR8P0PlaneArray::get_tap_var0(int srcPara, int destPara) {
  int retTap = 0;
  ...
  retTap = (8 - retTap) / 2;
  // retTap = 3 is null for htap & vtap on r8p0

  return retTap;
}

bool GspR8P0PlaneArray::miscCfgParcel(int mode_type, uint32_t transform) {
  bool status = false;
  int icnt = 0;
  struct gsp_r8p0_misc_cfg_user &r8p0_misc_cfg = mConfigs[mConfigIndex].misc;
  struct gsp_r8p0_img_layer_user r8p0_limg_cfg;
  uint32_t dstw = 0;
  uint32_t dsth = 0;
  ...
  case 0: {
    ...
    r8p0_misc_cfg.work_mod = 0;
    r8p0_misc_cfg.core_num = 0;

    r8p0_misc_cfg.workarea_src_rect.st_x = 0;
    r8p0_misc_cfg.workarea_src_rect.st_y = 0;
    r8p0_misc_cfg.workarea_src_rect.rect_w = mOutputWidth;
    r8p0_misc_cfg.workarea_src_rect.rect_h = mOutputHeight;
  }
    status = true;
    break;
  default:
    ALOGE("gsp r8p0 not implement other mode(%d) yet! ", mode_type);
    break;
  }
...
int GspR8P0PlaneArray::split(SprdHWLayer **list, int count) {
...
int GspR8P0PlaneArray::rotAdjustSingle(uint16_t *dx, uint16_t *dy, uint16_t *dw,
...
int GspR8P0PlaneArray::rotAdjust(struct gsp_r8p0_cfg_user *cmd_info,
  ...
  uint32_t transform = 0;
  int32_t icnt = 0;
  struct gsp_r8p0_osd_layer_user *osd_info = NULL;
  struct gsp_r8p0_img_layer_user *img_info = NULL;
...
bool GspR8P0PlaneArray::parcel(SprdUtilSource *source, SprdUtilTarget *target) {
...
int GspR8P0PlaneArray::set(SprdUtilSource *source, SprdUtilTarget *target,
  ...
  if (parcel(source, target) == false) {
    ALOGE("gsp r8p0plane parcel failed");
    return ret;
  }
