...
GspR8P0PlaneOsd::GspR8P0PlaneOsd(bool async, const GspRangeSize &range) {
  mIndex = -1;
  mAttached = false;
  mAsync = async;
  mRangeSize = range;
  mZorder = 0;
  mRangeSize.print();
  ALOGI("create GspR8P0PlaneOsd");
}

void GspR8P0PlaneOsd::reset(int flag) {
  mAttached = false;
  mIndex = -1;
  mDebugFlag = flag;
  memset(&mConfig, 0, sizeof(struct gsp_r8p0_osd_layer_user));
}

bool GspR8P0PlaneOsd::checkFBC(uint32_t pitch, uint32_t height,
                               enum gsp_r8p0_osd_layer_format format,
                               bool inFBC) {
  bool isAFBC = false;
  uint32_t horAlign, verAlign;

  if (inFBC) {
    if ((GSP_R8P0_OSD_FMT_ARGB888 == format) ||
        (GSP_R8P0_OSD_FMT_RGB888 == format) ||
        (GSP_R8P0_OSD_FMT_RGB565 == format)) {
      isAFBC = true;
      horAlign = 16;
      verAlign = 16;
    } else {
...
bool GspR8P0PlaneOsd::adapt(SprdHWLayer *layer, int LayerIndex) {
  ...
  enum gsp_r8p0_osd_layer_format format = osdFormatConvert(ADP_FORMAT(handle));
...
void GspR8P0PlaneOsd::configCommon(int wait_fd, int share_fd, int enable) {
...
void GspR8P0PlaneOsd::configSize(struct sprdRect *srcRect,
                                 struct sprdRect *dstRect, uint32_t pitch,
                                 uint32_t height) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
...
void GspR8P0PlaneOsd::configSize(uint32_t w, uint32_t h) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
...
void GspR8P0PlaneOsd::configFormat(enum gsp_r8p0_osd_layer_format format) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
...
void GspR8P0PlaneOsd::configFBC(bool inFBC, int format,
                                native_handle_t *handle) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
  uint32_t fbc_mode;

  if (inFBC && ((GSP_R8P0_OSD_FMT_ARGB888 == format) ||
                (GSP_R8P0_OSD_FMT_RGB888 == format) ||
                (GSP_R8P0_OSD_FMT_RGB565 == format)))
    fbc_mode = 1;
  else
...
void GspR8P0PlaneOsd::configPmargbMode(uint32_t blendMode) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
...
void GspR8P0PlaneOsd::configAlpha(uint8_t alpha) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
...
void GspR8P0PlaneOsd::configZorder(uint8_t zorder) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
...
void GspR8P0PlaneOsd::configEndian(native_handle_t *handle) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
  ...
  params->endian.y_rgb_qword_endn = GSP_R8P0_QWORD_ENDN_0;
...
void GspR8P0PlaneOsd::configPallet(int enable) {
  struct gsp_r8p0_osd_layer_params *params = &mConfig.params;

  params->pallet_en = enable;
}

struct gsp_r8p0_osd_layer_user &GspR8P0PlaneOsd::getConfig() {
  return mConfig;
}

bool GspR8P0PlaneOsd::parcel(SprdHWLayer *layer) {
  if (layer == NULL) {
...
bool GspR8P0PlaneOsd::parcel(uint32_t w, uint32_t h) {
  configPallet(1);
