...
int GspLiteR1P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
                                 uint32_t h) {
  if (mPreDpuFlag)
    return partial_adapt(list, count, w, h);
  else
    return full_adapt(list, count, w, h);
}

int GspLiteR1P0PlaneArray::full_adapt(SprdHWLayer **list, int count, uint32_t w,
                                      uint32_t h) {
  int ret = 0;
  bool result = false;
  ...
  while (i < count) {
    ...
    if ((ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YCbCr_420_SP) ||
        ...
        (ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YCbCr_422_SP) ||
        (ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YCrCb_422_SP)) {
      yuv_index = i;
      break;
    }
    i++;
...
int GspLiteR1P0PlaneArray::partial_adapt(SprdHWLayer **list, int count,
  ...
  for (int i = 0; i < count; i++) {
    ...
    if ((ADP_FORMAT(privateH) == HAL_PIXEL_FORMAT_YCbCr_420_SP) ||
      ...
      ALOGI_IF(mDebugFlag, "gsp lite_r1p0 adapt success");
      return ret;
    } else {
      continue;
    }
  ...
  ALOGI_IF(mDebugFlag, "lite_r1p0  unsupport case");
  return -1;
}
...
bool GspLiteR1P0PlaneArray::dstPlaneParcel(native_handle_t *handle, uint32_t w,
...
void GspLiteR1P0PlaneArray::bkAdjust(struct gsp_lite_r1p0_cfg_user *cmd_info) {
...
bool GspLiteR1P0PlaneArray::needDstFormatAdjust() {
  int32_t dst_x = 0;
  int32_t dst_y = 0;
  int32_t dst_w = 0;
  struct gsp_lite_r1p0_img_layer_user imglayer;

  ALOGI_IF(mDebugFlag,
           "gsp workaround for dpu doesn't support yuv OddBoundary");
