 ...
 */
#ifndef GSPR7P0PLANE_GSPR7P0PLANEDST_H_
#define GSPR7P0PLANE_GSPR7P0PLANEDST_H_
...
#define R7P0_DST_GSP_MMU_SIZE (16 * 1024 * 1024)

namespace android {

class GspR7P0PlaneDst : public GspR7P0Plane {
public:
  explicit GspR7P0PlaneDst(bool async, int max_gspmmu_size,
                           int max_gsp_bandwidth);
  virtual ~GspR7P0PlaneDst() {}

  bool adapt(SprdHWLayer **list, int count);

  struct gsp_r7p0_des_layer_user &getConfig();
...
private:
  enum gsp_r7p0_des_layer_format dstFormatConvert(int format);
  ...
  bool configOutputRotation(enum gsp_rot_angle rot_angle, bool outIFBC, int format);
  ...
  struct gsp_r7p0_des_layer_user mConfig;
...
#endif // GSPR7P0PLANE_GSPR7P0PLANEDST_H_
