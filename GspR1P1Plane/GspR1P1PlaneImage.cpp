...
GspR1P1PlaneImage::GspR1P1PlaneImage(bool async, bool oddSupport,
                                     bool videoSize, uint16_t up, uint16_t down,
...
bool GspR1P1PlaneImage::checkScale(struct sprdRect *srcRect,
  ...
  if (checkScaleDown(srcRect, dstRect, rot) == false)
    return false;
...
bool GspR1P1PlaneImage::adapt(SprdHWLayer *layer, int index) {
  ...
  mali_gralloc_yuv_info yuv_info = (mali_gralloc_yuv_info)ADP_YINFO(handle);
  //  enum gsp_addr_type bufType = bufTypeConvert(ADP_FLAGS(handle));
  enum gsp_r1p1_src_layer_format format = srcFormatConvert(ADP_FORMAT(handle));
  ...
  if (checkYuvType(yuv_info) == false)
    return false;

  if (checkVideoSize(srcRect) == false)
    return false;

  if (checkOddBoundary(srcRect) == false)
    return false;
...
void GspR1P1PlaneImage::configMisc(native_handle_t *handle,
                                   enum gsp_r1p1_src_layer_format format,
                                   enum gsp_rot_angle rot) {
  struct gsp_r1p1_img_layer_params *params = &mConfig.params;

  if (checkYuvInfo((mali_gralloc_yuv_info)ADP_YINFO(handle)) == true) {
    params->y2r_opt = 1;
  }

  if (isVideoLayer(format) == true && needTransform(rot) == true) {
    params->bnd_bypass = 1; // means no split
  } else {
    params->bnd_bypass = 0; // means split
  }
  ALOGI_IF(mDebugFlag, "config y2r_opt: %d, bnd_bypass: %d", params->y2r_opt,
           params->bnd_bypass);
}

void GspR1P1PlaneImage::configPmargbMode(uint32_t blendMode) {
  struct gsp_r1p1_img_layer_params *params = &mConfig.params;
...
void GspR1P1PlaneImage::configEndian(native_handle_t *handle) {
  struct gsp_r1p1_img_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
