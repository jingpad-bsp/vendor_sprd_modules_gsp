...
bool GspR1P1Plane::checkBlending(SprdHWLayer *layer) {
  bool result = true;
  ...
  case HWC_BLENDING_COVERAGE:
    ALOGI_IF(mDebugFlag, "not support blending flag: %d",
             layer->getBlendMode());
    result = true;
    break;
  default:
    ALOGE("not support blending flag: %d", layer->getBlendMode());
  }
...
bool GspR1P1Plane::isVideoLayer(enum gsp_r1p1_src_layer_format format) {
  ...
  if (format > GSP_SRC_FMT_RGB565 && format < GSP_SRC_FMT_8BPP)
    result = true;
