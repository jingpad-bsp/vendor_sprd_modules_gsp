...
int GspR3P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
                             uint32_t h) {
  int ret = 0;
  bool result = false;
  ...
  if (w > 1080 && h > 1920) {
    ALOGI_IF(mDebugFlag,
             "Filter GSP do QHD composition to avoid Dispc underflow");
    return -1;
  }

  int j = 0;
  while (j < count) {
    privateH = list[j]->getBufferHandle();
    if (ADP_COMPRESSED(privateH)) {
...
bool GspR3P0PlaneArray::dstPlaneParcel(native_handle_t *handle, uint32_t w,
