...
GspR3P0PlaneImage::GspR3P0PlaneImage(bool async, bool is_need_even,
  ...
  ALOGI("create GspR3P0PlaneImage");
  mRangeSize.print();
...
bool GspR3P0PlaneImage::checkYuvInfo(mali_gralloc_yuv_info info) {
  bool result = true;
...
bool GspR3P0PlaneImage::checkVideoSize(struct sprdRect *srcRect) {
  bool result = true;
...
bool GspR3P0PlaneImage::checkOddBoundary(
    struct sprdRect *srcRect, enum gsp_r3p0_img_layer_format format) {
  bool result = true;
...
bool GspR3P0PlaneImage::needScaleUpDown(struct sprdRect *srcRect,
...
bool GspR3P0PlaneImage::checkScaleSize(struct sprdRect *srcRect,
                                       struct sprdRect *dstRect,
                                       enum gsp_rot_angle rot) {
  bool result = true;

  uint16_t scaleUpLimit = mScaleRangeUp / 64;
...
bool GspR3P0PlaneImage::checkScale(struct sprdRect *srcRect,
                                   struct sprdRect *dstRect,
                                   enum gsp_rot_angle rot) {
...
bool GspR3P0PlaneImage::checkImgLayerFormat(
    enum gsp_r3p0_img_layer_format format) {
  bool result = true;

  if (format == GSP_R3P0_IMG_FMT_MAX_NUM) {
...
void GspR3P0PlaneImage::reset(int flag) {
  mAttached = false;
  mIndex = -1;
  mDebugFlag = flag;
  memset(&mConfig, 0, sizeof(struct gsp_r3p0_img_layer_user));
}

bool GspR3P0PlaneImage::adapt(SprdHWLayer *layer, int LayerIndex) {
  ...
  enum gsp_r3p0_img_layer_format format = imgFormatConvert(ADP_FORMAT(handle));
...
void GspR3P0PlaneImage::configCommon(int wait_fd, int share_fd) {
...
void GspR3P0PlaneImage::configScaleEnable(struct sprdRect *srcRect,
                                          struct sprdRect *dstRect,
                                          enum gsp_rot_angle rot) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
...
void GspR3P0PlaneImage::configSize(struct sprdRect *srcRect,
...
void GspR3P0PlaneImage::configFormat(enum gsp_r3p0_img_layer_format format) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
...
void GspR3P0PlaneImage::configPmargbMode(uint32_t blendMode) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
...
void GspR3P0PlaneImage::configAlpha(uint8_t alpha) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
...
void GspR3P0PlaneImage::configCSCMatrix(uint8_t y2r_mod) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
...
void GspR3P0PlaneImage::configZorder(uint8_t zorder) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
...
void GspR3P0PlaneImage::configEndian(native_handle_t *handle) {
  struct gsp_r3p0_img_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
  ...
  case HAL_PIXEL_FORMAT_YV12: // YUV420_3P, Y V U
    common->offset.uv_offset +=
        ALIGN(ADP_STRIDE(handle) / 2, 16) * ADP_HEIGHT(handle) / 2;
    break;
  case HAL_PIXEL_FORMAT_YCrCb_420_SP:
    params->endian.uv_word_endn = GSP_R3P0_WORD_ENDN_3;
    break;
  ...
  ALOGI_IF(
      mDebugFlag,
      "image plane y_rgb_word_endn: %d, uv_word_endn: %d, va_word_endn: %d",
...
struct gsp_r3p0_img_layer_user &GspR3P0PlaneImage::getConfig() {
  return mConfig;
}

bool GspR3P0PlaneImage::parcel(SprdHWLayer *layer) {
  if (layer == NULL) {
  ...
  uint8_t y2r_mod = 0;
  if (ADP_YINFO(handle) == MALI_YUV_BT601_NARROW ||
      ADP_YINFO(handle) == MALI_YUV_BT709_NARROW
      /*||private_h->yuv_info == MALI_YUV_BT2020_NARROW*/) {
    y2r_mod = 3;
  } else if (ADP_YINFO(handle) == MALI_YUV_BT601_WIDE ||
             ADP_YINFO(handle) == MALI_YUV_BT709_WIDE
