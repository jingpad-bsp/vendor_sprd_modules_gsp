 ...
 */
#ifndef GSPLiteR3P0PLANE_GSPLiteR3P0PLANEDST_H_
#define GSPLiteR3P0PLANE_GSPLiteR3P0PLANEDST_H_
...
#define LITE_R3P0_DST_GSP_MMU_SIZE (16 * 1024 * 1024)

namespace android {

class GspLiteR3P0PlaneDst : public GspLiteR3P0Plane {
public:
  explicit GspLiteR3P0PlaneDst(bool async, int max_gspmmu_size,
                               int max_gsp_bandwidth);
  virtual ~GspLiteR3P0PlaneDst() {}

  bool adapt(SprdHWLayer **list, int count);

  struct gsp_lite_r3p0_des_layer_user &getConfig();
...
private:
  enum gsp_lite_r3p0_des_layer_format dstFormatConvert(int format);
  ...
  struct gsp_lite_r3p0_des_layer_user mConfig;
...
#endif // GSPLiteR3P0PLANE_GSPLiteR3P0PLANEDST_H_
