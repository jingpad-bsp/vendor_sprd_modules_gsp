...
GspR6P0PlaneImage::GspR6P0PlaneImage(bool async, bool is_need_even,
  ...
  ALOGI("create GspR6P0PlaneImage");
  mRangeSize.print();
...
#if 0
bool GspR6P0PlaneImage::checkYuvInfo(mali_gralloc_yuv_info info) {
  bool result = true;
...
bool GspR6P0PlaneImage::checkVideoSize(struct sprdRect *srcRect) {
  bool result = true;
...
bool GspR6P0PlaneImage::checkOddBoundary(
    struct sprdRect *srcRect, enum gsp_r6p0_img_layer_format format) {
  bool result = true;
...
bool GspR6P0PlaneImage::needScaleUpDown(struct sprdRect *srcRect,
...
bool GspR6P0PlaneImage::checkScaleSize(struct sprdRect *srcRect,
                                       struct sprdRect *dstRect,
...
bool GspR6P0PlaneImage::checkScale(struct sprdRect *srcRect,
                                   struct sprdRect *dstRect,
                                   enum gsp_rot_angle rot, bool inFBC) {
...
bool GspR6P0PlaneImage::checkImgLayerFormat(
    enum gsp_r6p0_img_layer_format format) {
  bool result = true;

  if (format == GSP_R6P0_IMG_FMT_MAX_NUM) {
...
void GspR6P0PlaneImage::reset(int flag) {
  mAttached = false;
  mIndex = -1;
  mDebugFlag = flag;
  memset(&mConfig, 0, sizeof(struct gsp_r6p0_img_layer_user));
}

bool GspR6P0PlaneImage::checkFBC(uint32_t pitch, uint32_t height,
                                 enum gsp_r6p0_img_layer_format format,
                                 bool inFBC) {
  bool isIFBC = false;
  ...
  if (inFBC) {
    if ((GSP_R6P0_IMG_FMT_ARGB888 == format) ||
        (GSP_R6P0_IMG_FMT_RGB888 == format) ||
        (GSP_R6P0_IMG_FMT_RGB565 == format)) {
      isIFBC = true;
      if (GSP_R6P0_IMG_FMT_RGB565 == format)
        horAlign = 16;
      else
        horAlign = 8;
...
bool GspR6P0PlaneImage::checkInputRotation(
  ...
  if (rot != 0) {
    switch (format) {
    case GSP_R6P0_IMG_FMT_YUV420_2P:
    case GSP_R6P0_IMG_FMT_YV12:
    case GSP_R6P0_IMG_FMT_ARGB888:
    case GSP_R6P0_IMG_FMT_RGB888:
    case GSP_R6P0_IMG_FMT_RGB565:
      break;
    default:
...
bool GspR6P0PlaneImage::adapt(SprdHWLayer *layer, int LayerIndex,
                              bool InputRotMode) {
  ...
  enum gsp_r6p0_img_layer_format format = imgFormatConvert(ADP_FORMAT(handle));
...
void GspR6P0PlaneImage::configCommon(int wait_fd, int share_fd, int enable) {
...
int GspR6P0PlaneImage::get_tap_var0(int srcPara, int destPara) {
  int retTap = 0;
...
void GspR6P0PlaneImage::configScale(struct sprdRect *srcRect,
                                    struct sprdRect *dstRect,
                                    enum gsp_rot_angle rot) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
  uint32_t dstw = 0;
  uint32_t dsth = 0;
...
void GspR6P0PlaneImage::configSize(struct sprdRect *srcRect,
...
void GspR6P0PlaneImage::configFormat(enum gsp_r6p0_img_layer_format format) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
...
void GspR6P0PlaneImage::configFBC(bool inFBC, int format,
                                  native_handle_t *handle) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
  uint32_t fbc_mode;

  if (inFBC && ((GSP_R6P0_IMG_FMT_ARGB888 == format) ||
                (GSP_R6P0_IMG_FMT_RGB888 == format) ||
                (GSP_R6P0_IMG_FMT_RGB565 == format)))
    fbc_mode = 1;
  else
...
void GspR6P0PlaneImage::configPmargbMode(uint32_t blendMode) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
...
void GspR6P0PlaneImage::configAlpha(uint8_t alpha) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
...
void GspR6P0PlaneImage::configCSCMatrix(native_handle_t *handle) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
  uint8_t y2r_mod = 3;
...
void GspR6P0PlaneImage::configY2Y(uint8_t y2y_mod) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
...
void GspR6P0PlaneImage::configZorder(uint8_t zorder) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
...
void GspR6P0PlaneImage::configEndian(native_handle_t *handle) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;
  struct gsp_layer_user *common = &mConfig.common;
  ...
  case HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED:
    params->endian.uv_word_endn = GSP_R6P0_WORD_ENDN_3;
    break;
...
void GspR6P0PlaneImage::configInputRot(enum gsp_rot_angle rot) {
  struct gsp_r6p0_img_layer_params *params = &mConfig.params;

  params->rot_angle = rot;
  ALOGI_IF(mDebugFlag, "config input rotation : 0x%d", rot);
}
struct gsp_r6p0_img_layer_user &GspR6P0PlaneImage::getConfig() {
  return mConfig;
}

bool GspR6P0PlaneImage::parcel(SprdHWLayer *layer, bool InputRotMode) {
  if (layer == NULL) {
...
bool GspR6P0PlaneImage::parcel(uint32_t w, uint32_t h) {
