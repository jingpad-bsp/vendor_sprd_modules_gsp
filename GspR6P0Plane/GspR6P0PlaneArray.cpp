...
#include "GspR6P0PlaneArray.h"
#include <algorithm>
#include "GspR6P0PlaneImage.h"
#include "GspR6P0PlaneOsd.h"
...
GspR6P0PlaneArray::GspR6P0PlaneArray(bool async) {
  mAsync = async;
  mSplitSupport = 0;
  mDebugFlag = 0;
  mInputRotMode = false;
...
GspR6P0PlaneArray::~GspR6P0PlaneArray() {
  for (int i = 0; i < R6P0_IMGL_NUM; i++) {
    if (mImagePlane[i])
      delete mImagePlane[i];
  }
  for (int i = 0; i < R6P0_OSDL_NUM; i++) {
    if (mOsdPlane[i])
      delete mOsdPlane[i];
  }
...
int GspR6P0PlaneArray::init(int fd) {
  int ret = -1;
  struct drm_gsp_capability drm_cap;

  drm_cap.size = sizeof(gsp_r6p0_capability);
  drm_cap.cap = &mCapability;

  ret = drmIoctl(fd, DRM_IOCTL_SPRD_GSP_GET_CAPABILITY, &drm_cap);
  if (ret < 0) {
    ALOGE("gspr6p0plane array get capability failed, ret=%d.\n", ret);
    return ret;
  }
  ...
  for (int i = 0; i < R6P0_IMGL_NUM; i++) {
    mImagePlane[i] = new GspR6P0PlaneImage(
    ...
    if (mImagePlane[i] == NULL) {
      ALOGE("R6P0 ImgPlane alloc fail!");
      return -1;
    }
  }

  for (int i = 0; i < R6P0_OSDL_NUM; i++) {
    mOsdPlane[i] = new GspR6P0PlaneOsd(mAsync, range);
    if (mOsdPlane[i] == NULL) {
      ALOGE("R6P0 OsdPlane[%d] alloc fail!", i);
      return -1;
    }
  }

  mDstPlane = new GspR6P0PlaneDst(mAsync, mCapability.max_gspmmu_size,
                                  mCapability.max_gsp_bandwidth);
  if (mDstPlane == NULL) {
    ALOGE("R6P0 DstPlane alloc fail!");
    return -1;
  }

  mPlaneNum = R6P0_IMGL_NUM + R6P0_OSDL_NUM;

  mConfigs = new gsp_r6p0_cfg_user[mCapability.common.io_cnt];
  if (mConfigs == NULL) {
    ALOGE("R6P0 mConfigs alloc fail, io_cnt:%d !", mCapability.common.io_cnt);
    return -1;
  }

  drm_mConfigs =
      (struct drm_gsp_r6p0_cfg_user *)malloc(sizeof(drm_gsp_r6p0_cfg_user));
  drm_mConfigs->async = mAsync;
  drm_mConfigs->config = mConfigs;
  drm_mConfigs->size = sizeof(gsp_r6p0_cfg_user);
  drm_mConfigs->num = 1;
...
void GspR6P0PlaneArray::reset() {
  ...
  memset(&mConfigs->misc, 0, sizeof(struct gsp_r6p0_misc_cfg_user));
}

void GspR6P0PlaneArray::queryDebugFlag(int *debugFlag) {
...
bool GspR6P0PlaneArray::checkLayerCount(int count) {
  return count > mPlaneNum ? false : true;
}

bool GspR6P0PlaneArray::isExhausted() {
  return mAttachedNum == mMaxAttachedNum ? true : false;
}

bool GspR6P0PlaneArray::isSupportBufferType(enum gsp_addr_type type) {
...
bool GspR6P0PlaneArray::isSupportSplit() { return mSplitSupport; }

void GspR6P0PlaneArray::planeAttached() {
  mAttachedNum++;
  ALOGI_IF(mDebugFlag, "has attached total %d plane", mAttachedNum);
}

bool GspR6P0PlaneArray::isImagePlaneAttached(int ImgPlaneIndex) {
  return mImagePlane[ImgPlaneIndex]->isAttached();
}

bool GspR6P0PlaneArray::isOsdPlaneAttached(int OsdPlaneIndex) {
  return mOsdPlane[OsdPlaneIndex]->isAttached();
}

bool GspR6P0PlaneArray::imagePlaneAdapt(SprdHWLayer *layer, int ImgPlaneIndex,
...
bool GspR6P0PlaneArray::osdPlaneAdapt(SprdHWLayer *layer, int OsdPlaneIndex,
                                      int LayerIndex) {
...
bool GspR6P0PlaneArray::dstPlaneAdapt(SprdHWLayer **list, int count) {
  return mDstPlane->adapt(list, count);
}

bool GspR6P0PlaneArray::needImgLayer(SprdHWLayer *layer) {
...
bool GspR6P0PlaneArray::needRotation(SprdHWLayer *layer) {
  uint32_t transform = layer->getTransform();

  return transform == 0 ? false : true;
}

int GspR6P0PlaneArray::adapt(SprdHWLayer **list, int count, uint32_t w,
                             uint32_t h) {
  int DPULayerNum = 0;
  int i = 0;
  int ret = 0;
...
int GspR6P0PlaneArray::full_adapt(SprdHWLayer **list, int count, uint32_t w,
                                  uint32_t h) {
  int ret = 0;
  bool result = false;
  ...
  for (i = 0; i < count; i++) {
    if (needImgLayer(list[i]) == true) {
      ...
      if (img_index0 < R6P0_IMGL_NUM - yuv_index) {
  ...
  if (result == false) {
    if (!input_rot_adapt(list, count)) {
      mInputRotMode = true;
      ALOGI_IF(mDebugFlag, "gsp r6p0 input rotation mode.");
    } else {
  ...
  ALOGI_IF(mDebugFlag, "gsp r6p0plane full adapt success");

  return ret;
}

int GspR6P0PlaneArray::partial_adapt(SprdHWLayer **list, int count, uint32_t w,
                                     uint32_t h) {
  int ret = 0;
  bool result = false;
  native_handle_t *privateH = NULL;
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (list[i]->getAccelerator() != ACCELERATOR_DISPC) {
      ...
      img_index++;
      ALOGI_IF(mDebugFlag, "gsp r6p0 partial adapt success");
      return 0;
    }
  }

  ALOGI_IF(mDebugFlag, "gsp r6p0 partial adapt: strange to go here!");
  return ret;
}

int GspR6P0PlaneArray::input_rot_adapt(SprdHWLayer **list, int count) {
  int ret = 0;
  bool result = false;
  int32_t yuv_index = 0; // yuv layer index
  int32_t osd_index = 0;
  ...
  ALOGI_IF(mDebugFlag, "gsp r6p0 input rotation adapt success");
  return ret;
}

bool GspR6P0PlaneArray::imagePlaneParcel(SprdHWLayer **list, int ImgPlaneIndex,
                                         bool InputRotMode) {
...
bool GspR6P0PlaneArray::osdPlaneParcel(SprdHWLayer **list, int OsdPlaneIndex) {
...
bool GspR6P0PlaneArray::dstPlaneParcel(native_handle_t *handle, uint32_t w,
                                       uint32_t h, int format, int wait_fd,
                                       int32_t angle) {
  return mDstPlane->parcel(handle, w, h, format, wait_fd, angle);
}

int GspR6P0PlaneArray::get_tap_var0(int srcPara, int destPara) {
  int retTap = 0;
  ...
  retTap = (8 - retTap) / 2;
  // retTap = 3 is null for htap & vtap on r6p0

  return retTap;
}

bool GspR6P0PlaneArray::miscCfgParcel(int mode_type, uint32_t transform) {
  bool status = false;
  int icnt = 0;
  struct gsp_r6p0_misc_cfg_user &r6p0_misc_cfg = mConfigs[mConfigIndex].misc;
  struct gsp_r6p0_img_layer_user r6p0_limg_cfg;
  uint32_t dstw = 0;
  uint32_t dsth = 0;
  ...
  case 0: {
    ...
    r6p0_misc_cfg.work_mod = 0;
    r6p0_misc_cfg.core_num = 0;

    r6p0_misc_cfg.workarea_src_rect.st_x = 0;
    r6p0_misc_cfg.workarea_src_rect.st_y = 0;
    r6p0_misc_cfg.workarea_src_rect.rect_w = mOutputWidth;
    r6p0_misc_cfg.workarea_src_rect.rect_h = mOutputHeight;
  }
    status = true;
    break;
  default:
    ALOGE("gsp r6p0 not implement other mode(%d) yet! ", mode_type);
    break;
  }
...
int GspR6P0PlaneArray::split(SprdHWLayer **list, int count) {
...
int GspR6P0PlaneArray::rotAdjustSingle(uint16_t *dx, uint16_t *dy, uint16_t *dw,
...
int GspR6P0PlaneArray::rotAdjust(struct gsp_r6p0_cfg_user *cmd_info,
  ...
  int32_t icnt = 0;
  struct gsp_r6p0_osd_layer_user *osd_info = NULL;
  struct gsp_r6p0_img_layer_user *img_info = NULL;
...
bool GspR6P0PlaneArray::parcel(SprdUtilSource *source, SprdUtilTarget *target) {
...
int GspR6P0PlaneArray::set(SprdUtilSource *source, SprdUtilTarget *target,
  ...
  if (parcel(source, target) == false) {
    ALOGE("gsp r6p0plane parcel failed");
    return ret;
  }
